...
export default function NavBar({ scrollTo, isStart }) {
  ...
  function tl_Main() {
    ...
    tl.fromTo(barMain.current.children[0], { opacity: 0 }, { opacity: 1, duration: 0.3 });
  }

  return (
    <div className="w-full h-24 sm:h-20 flex justify-between items-center pt-8 sm:py-8 pr-20 pl-10 sm:px-7">
      <a href="/" target="_blank" className="font-PP-Right-Didone font-light text-[40px] text-sec-3 transition duration-300 hover:text-pr-2">
        VIBE 400
      </a>
      <div ref={barMain} className="bg-sec-3 max-w-[500px] text-gray-black h-14 rounded-full text-body-2 sm:hidden">
        <div className="w-full h-full flex items-center justify-between px-10 gap-6">
          <div className="flex justify-between grow">
            <button onClick={click("about")} className="hover:text-pr-2 transition duration-300">
              About
            </button>
            <button onClick={click("story")} className="hover:text-pr-2 transition duration-300">
              Story
            </button>
            <button onClick={click("gallery")} className="hover:text-pr-2 transition duration-300">
              Gallery
            </button>
          </div>
          /
          <button className="flex gap-1 group" onClick={click("contact")}>
            <svg className="fill-black group-hover:fill-pr-2 transition duration-300 w-[1rem]" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="group-hover:text-pr-2 transition duration-300">Contact</div>
          </button>
        </div>
      </div>
      <button className="w-[3rem] h-[2.625rem] bg-sec-3 rounded-full p-[1rem] hidden sm:flex" onClick={mButtonAnimation} ref={mButtonDiv}>
